...
import { getPastVaxDrives } from './../app-constants-apis';

export default function PastVaxDrives(props) {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchData = async () => {
    const resp = await fetch(getPastVaxDrives);
  ...
  return (
    <NativeBaseProvider mt={50}>
      <View style={styles.header}>
        <Text style={styles.header_name}>Past Vax-Drives</Text>
