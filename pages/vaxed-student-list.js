...
import { getVaccinatedStudents } from './../app-constants-apis';

export default function VaxedStudentList(props) {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchData = async () => {
    const resp = await fetch(getVaccinatedStudents);
  ...
  const renderItem = ({ item }) => {
    return (
      <Box px={5} py={2} rounded="md" bg="primary.300" my={2}>
        <View>
          <Text>{item.StudentID} | {item.Name} | Vaccine: {item.VaccineType} </Text>
        </View>
      </Box>
    );
  };
  ...
  return (
    <NativeBaseProvider mt={50}>
      <View style={styles.header}>
        <Text style={styles.header_name}>Vaccinated STUDENTS</Text>
