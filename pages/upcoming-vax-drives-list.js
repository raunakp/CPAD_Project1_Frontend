...
import { getUpcomingVaxDrives } from './../app-constants-apis';

export default function UpcomingVaxDrives(props) {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);

  const fetchData = async () => {
    const resp = await fetch(getUpcomingVaxDrives);
  ...
  return (
    <NativeBaseProvider mt={50}>
      <View style={styles.header}>
        <Text style={styles.header_name}>Upcoming Vax Drives</Text>
